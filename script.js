...
function detectWebGLContext () {
    ...
    if (gl && gl instanceof WebGLRenderingContext) {
        console.log("Selamat! Peramban Anda mendukung WebGL.");
        init();
    } else {
        alert("Gagal mendapatkan konteks WebGL. Browser atau perangkat Anda mungkin tidak mendukung WebGL.");
    }
}
...
async function init(){
    ...
    model = await tf.loadLayersModel('https://saepudinasep.github.io/Analisis-sentimen-TensorFlowjs/tfjs_model/model.json'); // Untuk VS Code Live Server
    // model = await tf.loadLayersModel('http://127.0.0.1:8887/tfjs_model/model.json');
    isModelLoaded = true;
 
    //Memanggil word_index
    const word_indexjson = await fetch('https://saepudinasep.github.io/Analisis-sentimen-TensorFlowjs/word_index.json'); // Untuk VS Code Live Server
    // const word_indexjson = await fetch('http://127.0.0.1:8887/word_index.json');
    word2index = await word_indexjson.json();
 
    console.log(model.summary());
    console.log('Model & Metadata Berhasil Dimuat');
}
// -----------------------------------
